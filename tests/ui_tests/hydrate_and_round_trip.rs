...
use crate::ui_tests::{
    ...
    wait_for_single, wait_for_text_contains, wait_for_text_not_contains, wait_for_visible,
};
...
impl BrowserTest<Context> for HydratesAndRoundTripsContent {
    ...
    async fn run(&self, driver: &WebDriver, context: &Context) -> Result<(), Report> {
        ...
        wait_for_visible(
            driver,
            "div#id.leptos-tiptap-instance.demo-editor[style*='display:block']",
        )
        .await?;
        ...
        wait_for_text_contains(driver, JSON_CONTENT_SELECTOR, "Lorem ipsum").await?;
